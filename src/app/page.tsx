...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-ps-black text-white relative overflow-hidden">

      {/* Navbar */}
      <nav className="sticky top-0 z-50 bg-ps-black/75 backdrop-blur-md px-4 py-4 border-b border-white/[0.05] flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <span className="font-black tracking-tighter text-xl text-white uppercase">PS DEALS</span>
        </div>
      </nav>
      ...
      <section className="px-4 py-12 md:py-24 max-w-4xl mx-auto relative">
        <div className="flex flex-col items-center justify-center text-center gap-8">
          <div className="animate-slide-up max-w-2xl flex flex-col items-center" style={{ animationDelay: '0.1s' }}>
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white mb-6 uppercase tracking-tight leading-[0.9]">
              Is it In Stock?
            </h1>
            <p className="text-lg md:text-xl text-zinc-400 mb-10 font-medium max-w-xl mx-auto leading-relaxed">
            ...
            {/* Coverage Info */}
            <div className="mt-8 p-4 bg-white/[0.02] rounded-2xl border border-white/10 w-full max-w-lg mx-auto backdrop-blur-sm">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {['Slim Disc', 'Slim Digital', 'Disc Standard', 'Digital Edition', 'FC26 Bundle', 'Fortnite Bundle', 'GTA V', 'Spider-Man 2', 'EA FC 24'].map((model) => (
                  <span key={model} className="text-[9px] font-bold bg-white/5 text-zinc-400 px-2.5 py-1 rounded-md border border-white/5">
                    {model}
                  </span>
                ))}
              </div>
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-3 text-[10px] font-black text-zinc-500 uppercase tracking-widest">
              <span className="flex items-center bg-white/5 px-3 py-2 rounded-xl border border-white/5"><ShieldCheck className="w-3.5 h-3.5 mr-2 text-ps-neon-blue" /> Verified Stores</span>
              <span className="flex items-center bg-white/5 px-3 py-2 rounded-xl border border-white/5"><Zap className="w-3.5 h-3.5 mr-2 text-ps-neon-blue" /> Instant Alerts</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Retailer List */}
      <section id="status" className="bg-black/25 border-t border-white/[0.03] py-20 px-4 scroll-mt-20">
